...
- `HOLDINGS_MFDATA_BASE_URL` — base URL for equity overlap analysis (`POST /api/mf/portfolio-stocks`). Default `https://mfdata.in`.
- `HOLDINGS_STOCK_OVERLAP_MAX_FUNDS` — number of largest MF positions to send to mfdata.in per analysis (default 20, max 40).
...
- `POST /api/mf/portfolio-stocks` — JSON `{ "positions": [ { "tradingsymbol", "fund?", "current_value" } ], "total_current" }` (1–50 rows). Resolves each MF ISIN via [mfdata.in](https://mfdata.in/docs) (`/search`, `/schemes/{code}`, `/families/{id}/holdings?holding_type=equity`), then returns **synthetic** portfolio-level equity weights (your % in each fund × each stock’s % inside that fund). Optional env: `HOLDINGS_MFDATA_BASE_URL` (default `https://mfdata.in`), `HOLDINGS_STOCK_OVERLAP_MAX_FUNDS` (default 20, max 40). Upstream timeouts return HTTP 502.
